...
import {
  ...
  Loader, Check, AlertCircle, CheckCircle, Info, XCircle
} from 'lucide-react';
...
export default function Explore() {
  ...
  const fetchCreations = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data.creations)) {
        const mappedCreations = data.creations.map(creation => ({
          ...creation,
        ...
        setAvailableLanguages([...new Set(mappedCreations.map(c => c.language).filter(Boolean))]);
        setAvailableTags([...new Set(mappedCreations.flatMap(c => c.tags || []).filter(Boolean))]);
      }
    } catch (err) {
      ...
      toast.info(
        <div className="flex items-center gap-5">
          ...
          <div className="flex-grow">
            <h4 className="text-white font-medium text-sm mb-1">Sign in required</h4>
            <p className="text-gray-400 text-xs">Add creations to your favorites</p>
          </div>
          <Link
            href="/auth/login"
            className="flex-shrink-0 px-3 py-1.5 text-xs font-medium bg-blue-500 hover:bg-blue-600
        ...
        </div>,
        {
          ...
          autoClose: 4000,
          closeButton: true,
          className: "backdrop-blur-md bg-gray-900/90 border border-gray-800/60 shadow-xl",
          progressClassName: "bg-blue-500 h-[2px]",
        }
      );
      return;
    }
  ...
  const StickyHeader = () => {
    ...
    return (
      ...
      >
        <div className="bg-gray-900/95 backdrop-blur-md border-b border-gray-800/60 shadow-xl pointer-events-auto">
          <div className="max-w-7xl mx-auto px-4 py-3">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center flex-1 gap-4 max-w-3xl mx-auto">
                <div className="flex-1 relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4" />
                  <input
                    type="text"
                    placeholder={isMobile ? "Search..." : "Search creations..."}
                    value={filters.search}
                    onChange={(e) => setFilters(prev => ({ ...prev, search: e.target.value }))}
                    className="w-full pl-9 pr-3 py-2 bg-gray-800/50 rounded-full border border-gray-700/50 
                        focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50 text-gray-300 text-sm 
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  className={`p-2 rounded-full transition-all duration-200 ${showFilters
                    ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20'
                    : 'bg-gray-800/70 text-gray-400 hover:text-white hover:bg-gray-700'
                    }`}
                >
  ...
  const ViewToggle = () => (
    <div className="flex gap-0.5 bg-gray-800/70 p-1 rounded-full">
      <button
        onClick={() => setViewMode('grid')}
        className={`p-1.5 rounded-full transition-all duration-200 ${viewMode === 'grid'
      ...
      <button
        ...
        className={`p-1.5 rounded-full transition-all duration-200 ${viewMode === 'list'
          ? 'bg-blue-500 text-white'
          : 'text-gray-400 hover:text-white hover:bg-gray-700/50'
          }`}
      >
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
                <div className="flex flex-wrap gap-3">
                  <div className="flex items-center gap-1.5 text-gray-400 text-sm">
                    ...
                    <Link
                      href={`/auth/user/profile/view/${creation.owner}`}
                      className="truncate max-w-[120px] hover:text-blue-400 transition-colors"
                    >
                      {creation.author || "Anonymous"}
                    </Link>
                  </div>
                  <div className="flex items-center gap-1.5 text-gray-400 text-sm">
  ...
  return (
    <>
      <ToastContainer
        ...
        toastClassName={() =>
          "relative flex p-4 mb-4 overflow-hidden rounded-xl backdrop-blur-md border border-gray-700/50 shadow-2xl bg-gradient-to-r from-gray-900/90 to-gray-800/90"
        }
        bodyClassName={() =>
          "flex items-center gap-3 text-sm font-medium ml-2 mr-2"
        }
        progressClassName={({ type }) => {
          ...
          return `h-[2px] ${colors[type] || colors.default}`;
        }}
      />
      <div className="relative">
        <Header user={user} className={isTopVisible ? 'relative' : 'invisible'} />
        <div ref={topRef} className="absolute top-0 left-0 right-0 h-1" />
        <StickyHeader />
      </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-8 border border-gray-700/50 shadow-xl">
                ...
                <h3 className="text-2xl font-bold text-gray-300 mb-3">Error Loading Creations</h3>
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-8 border border-gray-700/50 shadow-xl">
                <Code2 className="w-16 h-16 text-gray-600 mb-4 mx-auto" />
                <h3 className="text-2xl font-bold text-gray-300 mb-3">No Matching Creations</h3>
